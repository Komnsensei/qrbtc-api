...
// All DOI mints now flow through chain lineage
...
async function getPassportWithToken(pid) {
  var r = await db.from('passports').select('id, hex_id, revoked, zenodo_token').eq('id', pid).limit(1);
...
async function handleBead(req, res) {
  ...
  var passport = await getPassportWithToken(passport_id);
  if (!passport) return res.status(404).json({ error: 'Passport not found' });
  if (passport.revoked) return res.status(403).json({ error: 'Passport revoked' });
  var hexId = passport.hex_id || generateHexId(passport_id);
  ...
  var doiResult = null;
  try {
    doiResult = await mintBead({ _zenodo_token: passport.zenodo_token, _passport_id: passport_id, hex_id: hexId, bead_type: bead_type, bead_content: bead_content, session_context: body.session_context || null, tier: tierInfo.tier, total_degrees: totalDegrees });
  ...
  return res.status(200).json({ hex_id: hexId, bead_type: bead_type, bead_content: bead_content, tier: tierInfo.tier, total_degrees: totalDegrees, doi: doiResult && doiResult.ok ? { doi: doiResult.doi, doi_url: doiResult.doi_url, record_url: doiResult.record_url, polarity: '+', chain_position: doiResult.chain_position, chain_hash: doiResult.chain_hash } : null });
}
...
async function handleAward(req, res) {
  ...
  var passport = await getPassportWithToken(target_passport_id);
  if (!passport) return res.status(404).json({ error: 'Passport not found' });
  if (passport.revoked) return res.status(403).json({ error: 'Passport revoked' });
  var hexId = passport.hex_id || generateHexId(target_passport_id);
  ...
  var doiResult = null;
  try {
    doiResult = await mintAward({ _zenodo_token: passport.zenodo_token, _passport_id: target_passport_id, hex_id: hexId, award_name: award_name, award_reason: award_reason, degrees_bonus: degrees_bonus, tier: tierInfo.tier });
  } catch (e) { console.error('AWARD DOI:', e.message); }
  ...
  return res.status(200).json({ hex_id: hexId, award_name: award_name, award_reason: award_reason, degrees_bonus: degrees_bonus, tier: tierInfo.tier, doi: doiResult && doiResult.ok ? { doi: doiResult.doi, doi_url: doiResult.doi_url, polarity: '+', chain_position: doiResult.chain_position, chain_hash: doiResult.chain_hash } : null });
}
...
async function handleTrust(req, res) {
  ...
  var recPassport = await getPassportWithToken(recommender_id);
  var tgtPassport = await getPassportWithToken(target_passport_id);
  if (!tgtPassport) return res.status(404).json({ error: 'Target passport not found' });
  if (tgtPassport.revoked) return res.status(403).json({ error: 'Target passport revoked' });
  var recommenderHexId = recPassport.hex_id || generateHexId(recommender_id);
  var targetHexId = tgtPassport.hex_id || generateHexId(target_passport_id);
  ...
  var doiResult = null;
  try {
    doiResult = await mintTrustRec({ _zenodo_token: tgtPassport.zenodo_token, _passport_id: target_passport_id, hex_id: targetHexId, recommender_hex_id: recommenderHexId, recommender_tier: recTier.tier, recommendation_weight: weight, context: context });
  } catch (e) { console.error('TRUST DOI:', e.message); }
  ...
  return res.status(200).json({ target_hex_id: targetHexId, recommender_hex_id: recommenderHexId, recommender_tier: recTier.tier, weight: weight, context: context, doi: doiResult && doiResult.ok ? { doi: doiResult.doi, doi_url: doiResult.doi_url, polarity: '+', chain_position: doiResult.chain_position, chain_hash: doiResult.chain_hash } : null });
}
