...
async function isAdmin(passport_id) {
  try {
    var result = await db.from("passports").select("is_admin").eq("id", passport_id).limit(1);
...
async function isIPWhitelisted(ip_hash) {
  try {
    var result = await db.from("ip_whitelist").select("id").eq("ip_hash", ip_hash).eq("active", true).limit(1);
...
async function authenticate(req, requiredScope) {
  ...
  if (authHeader && authHeader.startsWith("Bearer ")) {
    ...
    if (jwtPayload) {
      ...
      logRequest({ passport_id: jwtPayload.passport_id, endpoint: endpoint, method: method, status_code: 200, latency_ms: latency, auth_type: "jwt", tier: jwtPayload.tier || "unknown", ip_hash: ip_hash });
      return { passport_id: jwtPayload.passport_id, tier: jwtPayload.tier || "unknown", scopes: jwtPayload.scopes || [], is_admin: isAdminUser, auth_type: "jwt" };
    }
  }
  ...
  var result = await db.from("api_keys").select("id, passport_id, scopes, tier, requests_used, requests_limit, revoked, last_request_at, request_window_start").eq("key_hash", key_hash).limit(1);
  ...
  if (!isAdminUser && used >= apiKey.requests_limit) {
    ...
    return { error: "Rate limit exceeded. Resets in " + Math.ceil((windowStart + WINDOW_MS - now) / 60000) + " minutes.", status: 429 };
  }
  ...
  await db.from("api_keys").update({ requests_used: used + 1, last_request_at: new Date(now).toISOString(), request_window_start: new Date(windowStart).toISOString() }).eq("id", apiKey.id);

  var latency = Date.now() - start;
  logRequest({ passport_id: apiKey.passport_id, endpoint: endpoint, method: method, status_code: 200, latency_ms: latency, auth_type: "api_key", tier: apiKey.tier, ip_hash: ip_hash });

  return { passport_id: apiKey.passport_id, tier: apiKey.tier, scopes: apiKey.scopes, requests_remaining: apiKey.requests_limit - used - 1, is_admin: isAdminUser, auth_type: "api_key" };
}
